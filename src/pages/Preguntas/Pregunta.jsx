...
function Preguntas() {
    ...
    const [preguntas, setPreguntasPorIdCuestionario] = useState(null);
    ...
    const fetchPreguntas = async () => {
        try {
            ...
            setPreguntasPorIdCuestionario(data);
        } catch (err) {
    ...
    function NuevaPregunta() {
        try {
            navigate(`/cuestionarios/${params.id_cuestionario}/NuevasPreguntas`);
        } catch (err) {
            setError(err.message);
        } finally {
            setLoading(false);
    ...
    function EditarPregunta(id_cuestionario, id_pregunta) {
        try {
            navigate(`/cuestionarios/${id_cuestionario}/pregunta/${id_pregunta}/EditarPreguntas`);
        } catch (err) {
            setError(err.message);
        } finally {
            setLoading(false);
                        ...
                        .map((pregunta) =>
                            <div className={'pregunta-item'} key={pregunta.id} onClick={() => Responder(pregunta.id_cuestionario, pregunta.id, pregunta.tipo)}
                                ...
                                <button
                                    ...
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        EditarPregunta(cuestionario.id, pregunta.id);
                                    }}
                                >
                                    Editar Pregunta
                                </button>
